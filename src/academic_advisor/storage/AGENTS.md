...
- Increment the knowledge revision on effective changes. Keep passage deletion linked to document deletion and reset the embedding signature when the knowledge base becomes empty.
- Search the active `knowledge_passages` across documents. Preserve document diversity when selecting results; do not revive the fixed-fixture catalog-only search.
